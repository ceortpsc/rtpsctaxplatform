...
test('AOL package manager: discovers workspaces from globs', async () => {
...
test('AOL package manager: installs parallel workspace links and writes RTPSC-package-lock.json', async () => {
  const root = await makeFixture();
  try {
...
test('AOL package manager: cache-hits when fingerprints and links are healthy', async () => {
  const root = await makeFixture();
  try {
...
test('AOL package manager: builds matching lock fingerprints', async () => {
  const root = await makeFixture();
  try {
    const manifest = await loadRootManifest(root);
    const workspaces = await discoverWorkspaces(root, manifest.workspaces);
...
test('AOL package manager: cli help and version exit 0', async () => {
...
test('AOL package manager: reads lockfile after install', async () => {
  const root = await makeFixture();
  try {
    await install(root, { quiet: true, force: true });
