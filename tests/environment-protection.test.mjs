...
test('platform identity is Ross Tax Pro Software Co Platform v2.0-dev', () => {
  ...
  assert.equal(PLATFORM_IDENTITY.productLine, 'Platform v2.0');
  assert.equal(PLATFORM_IDENTITY.release, 'v2.0-dev');
  assert.equal(PLATFORM_IDENTITY.version, '2.0.0-dev.0');
  assert.equal(PLATFORM_IDENTITY.channel, 'dev');
});
...
test('local environment is protected and blocks transmission by default', () => {
  ...
  assert.equal(result.release, 'v2.0-dev');
